...
const ScanScreen = ({ studentData, onBack }) => {
    ...
    const handleScan = async (result) => {
        ...
        try {
            ...
            console.log(`[STUDENT] Scan detected at ${scannedGateId}. Auto-Approvable: ${isAutoApprovable}, Limit Reached: ${limitReachedRef.current}`);
            ...
            if (currentLogId || isAutoApprovable) {
                const logUpdate = {
                    ...
                    status: limitReachedRef.current ? 'Denied' : (isAutoApprovable ? 'Success' : 'Pending')
                };
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 relative z-30 pb-20">
                <div className="w-full bg-[#1e1a17]/95 rounded-[40px] p-8 shadow-2xl border border-white/5 relative overflow-hidden backdrop-blur-2xl">
                    ...
                    {status === 'idle' || status === 'expired' ? (
                        ...
                        <div className="flex flex-col items-center text-center">
                            {/* Status Indicator */}
                            <div className={`flex items-center gap-2 mb-4 px-4 py-2 rounded-xl border ${status === 'approved' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                                }`}>
                                {status === 'approved' ? (
                            ...
                            <div className="w-full aspect-square relative rounded-[32px] overflow-hidden border-2 border-[#f47c20]/30 shadow-2xl mb-6 bg-black">
                                <div className={`absolute inset-0 border-4 transition-colors duration-300 z-30 pointer-events-none ${status === 'approved' ? 'border-amber-500/50' : 'border-emerald-500/50'
                                    }`} />
