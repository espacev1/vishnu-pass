...
const DashboardContent = ({ onNavigate }) => {
    ...
    const fetchDashboardData = useCallback(async (showLoading = true) => {
        ...
        try {
            ...
            const [studentsCount, guardsCount, scansCount, activityLogs, errorLogs, settings, activePassesCount, scansLast24h] = await Promise.all([
                supabase.from('students').select('*', { count: 'exact', head: true }),
                supabase.from('guards').select('*', { count: 'exact', head: true }),
                supabase.from('scan_sessions').select('*', { count: 'exact', head: true }),
                supabase.from('scan_sessions').select('*, students!scan_sessions_student_id_fkey(full_name, photo_url), guard_gates!scan_sessions_gate_id_fkey(name)').order('created_at', { ascending: false }).limit(5),
                supabase.from('scan_sessions').select('*', { count: 'exact', head: true }).in('status', ['rejected', 'denied', 'error', 'expired']).gt('created_at', last24h.toISOString()),
                supabase.from('portal_settings').select('updated_at').order('updated_at', { ascending: false }).limit(1),
                supabase.from('students').select('*', { count: 'exact', head: true }).eq('status', 'Active'),
                supabase.from('scan_sessions').select('*', { count: 'exact', head: true }).gt('created_at', last24h.toISOString())
            ]);

            // Calculate gate efficiency
            const hourlyScans = Math.round((scansLast24h.count || 0) / 24);
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            {(() => {
                                                ...
                                                const isAuth = ['success', 'completed', 'approved'].includes(s);
                                                ...
                                                return (
                                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full border ${isAuth ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : isPending ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                                                        ...
                                                        {isAuth ? 'Authorized' : isPending ? 'Pending' : 'Denied'}
                                                    </span>
                                                );
                                            })()}
                                        </td>
