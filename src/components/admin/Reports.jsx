                                ...
                                <div className="absolute top-full right-0 mt-2 w-36 bg-white border border-gray-100 rounded-xl shadow-lg shadow-gray-200/50 py-1 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                                    {[7, 5, 3, 2, 1].map((range) => (
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-2 text-xs font-bold transition-colors ${dateRange === range ? 'bg-orange-50 text-[#f47c20]' : 'text-gray-600 hover:bg-gray-50'
                                                }`}
                                        >
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {logs.map((log) => (
                            <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`inline-flex px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md ${(log.movement_type?.toUpperCase() === 'IN' || log.movement_type?.toUpperCase() === 'ENTRY') ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>
                                        {log.movement_type || 'N/A'}
                                    </span>
                                </td>
                                <td className="px-6 py-4">
