    ...
    const processSessionUpdate = useCallback(async (sessionId, source = 'Realtime') => {
        ...
        try {
            const { data: sessionInfo, error: sessionErr } = await supabase
                .from('scan_sessions')
                .select('student_id, gate_id, movement_type, warning, status')
                .eq('id', sessionId)
                .single();
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className={`w-1.5 h-1.5 rounded-full ${connectionStatus === 'safe' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' :
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase ${req.movement_type === 'IN' ? 'bg-emerald-100 text-emerald-600' :
                            ...
                            <div key={activity.id} className={`bg-white rounded-[28px] p-4 border border-white shadow-sm flex items-center justify-between group`}>
                                ...
                                <div className="flex flex-col items-end gap-1.5">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`text-[8px] font-black px-2.5 py-1 rounded-lg tracking-wider uppercase ${(activity.status === 'Success' || activity.status === 'completed' || activity.status === 'approved')
